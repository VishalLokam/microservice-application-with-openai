...
app.post("/product/buy", async (req, res) => {
  const { productsWithQuantity, userEmail, userAddress } = req.body;

  const productsForOrderDetails = [];
  for (productWithQuantity of productsWithQuantity) {
    const product = await Product.findOne({
      _id: { $in: productWithQuantity.id },
    }).lean();

    const productWithQuantityForOrder = {
      ...product,
      quantity: productWithQuantity.quantity,
    };

    productsForOrderDetails.push(productWithQuantityForOrder);
  }
  ...
  var order_response = {
    message:
      "Hooray!!! Order placed. Please wait for the confirmation email. This might take some time based on the order quantity",
  };
